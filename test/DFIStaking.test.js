...
describe("DFIStaking contract", () => {
    ...
    before(async () => {
        ...
        hhDFIStaking = await upgrades.deployProxy(
            dfiStaking,
            [erc20Token.target, bot.address, minDepAmount],
    ...
    describe("Transactions", () => {
        it("Should revert when set pause", async () => {
            await expect(hhDFIStaking.connect(addr1).pause()).to.be.revertedWith(ADMIN_ERROR);
        });
        ...
        it("Should revert when set unpause", async () => {
            await expect(hhDFIStaking.connect(addr1).unpause()).to.be.revertedWith(ADMIN_ERROR);
        });
        ...
        it("Should revert when set the bot address", async () => {
            await expect(hhDFIStaking.connect(addr1).setBotAddress(bot.address)).to.be.revertedWith(
                ADMIN_ERROR,
            );
        });
        ...
        it("Should revert when deposit paused=true", async () => {
            ...
            ).to.be.revertedWithCustomError(hhDFIStaking, "EnforcedPause");
            await hhDFIStaking.unpause();
        });
        ...
        it("Should revert when deposit msg.value < minimumDepositAmount", async () => {
            ...
            ).to.be.revertedWith("DFIStaking: invalid deposit amount");
        });
        ...
        it("Should revert when deposit not role for mint", async () => {
            ...
            ).to.be.revertedWithCustomError(erc20Token, "AccessControlUnauthorizedAccount");

            await erc20Token
                .connect(owner)
                .grantRole(ethers.encodeBytes32String("0x01"), hhDFIStaking.target);
        });
        ...
        it("Should deposit", async () => {
            ...
            await expect(await ethers.provider.getBalance(addr1.address)).to.equal(
                addr1BalanceBefore - amount - spentOnGas,
            );
            await expect(await ethers.provider.getBalance(bot.address)).to.equal(
                botBalanceBefore + amount,
            );
            await expect(await ethers.provider.getBalance(hhDFIStaking.target)).to.equal(
        ...
        it("Should revert when updateInvestment onlyBot", async () => {
            ...
            ).to.be.revertedWith("DFIStaking: only bot");
        });
        ...
        it("Should revert when updateInvestment userDeposit < _depositInfo[i].amount", async () => {
            ...
            ).to.be.revertedWith("DFIStaking: invalid deposit info");
        });
        ...
        it("Should updateInvestment - deposit > 0  investment = 0", async () => {
            ...
            await expect(await hhDFIStaking.userDeposit(addr1.address)).to.equal(
        ...
        it("Should updateInvestment - reinvest, userDeposit=0, rewards > minDepAmount", async () => {
            ...
            await expect(await hhDFIStaking.userInvestment(addr3.address)).to.equal(
                investmentBefore + amount,
            );
            await expect(await hhDFIStaking.userMintRewards(addr3.address)).to.equal(0);
            await expect(await erc20Token.balanceOf(addr3.address)).to.equal(
                stAmountBefore + amount,
            );
        });
        ...
        it("Should updateInvestment - deposit > 0  investment > 0  investment < deposit with rewards", async () => {
            ...
            await expect(await hhDFIStaking.userInvestment(addr1.address)).to.equal(
        ...
        it("Should updateInvestment with  diff users", async () => {
            ...
            await expect(await hhDFIStaking.userDeposit(addr2.address)).to.equal(
                userDepositBefore2 - investmentAmount2,
            );
            await expect(await hhDFIStaking.userDeposit(addr3.address)).to.equal(
                userDepositBefore3,
            );
            await expect(await hhDFIStaking.userDeposit(owner.address)).to.equal(
                userDepositBefore4 - investmentAmount4,
            );
            await expect(await hhDFIStaking.userInvestment(addr1.address)).to.equal(
                userInvestmentBefore1 + userDepositBefore1,
            );
            await expect(await hhDFIStaking.userInvestment(addr2.address)).to.equal(
                userInvestmentBefore2 + investmentAmount2,
            );
            await expect(await hhDFIStaking.userInvestment(addr3.address)).to.equal(
                userInvestmentBefore3 + rewardsAmount3,
            );
            await expect(await hhDFIStaking.userInvestment(owner.address)).to.equal(
        ...
        it("Should revert when request withdraw userInvestment[msg.sender] < _amount", async () => {
            ...
            ).to.be.revertedWith("DFIStaking: invalid amount for withdraw");
        });
        ...
        it("Should revert when updateWithdraw - userRequestedWithdraw < _withdraw.amount", async () => {
            ...
            await expect(hhDFIStaking.connect(bot).updateWithdraw(messageData)).to.be.revertedWith(
        ...
        it("Should updateWithdraw", async () => {
            ...
            const amount = ethers.parseEther("1");
            const userRequestedWithdrawBefore = await hhDFIStaking.userRequestedWithdraw(
                addr1.address,
            );
            ...
            await expect(await hhDFIStaking.userRequestedWithdraw(addr1.address)).to.equal(
                userRequestedWithdrawBefore - amount,
            );
            await expect(await hhDFIStaking.userClaimableAmount(addr1.address)).to.equal(
                userClaimableAmountBefore + amount,
            );
        });

        it("Should revert when withdraw _amount <= 0", async () => {
            await expect(hhDFIStaking.connect(addr3).withdraw()).to.be.revertedWith(
                "DFIStaking: invalid withdraw amount",
            );
        });

        it("Should revert when withdraw balance < _amount", async () => {
            await expect(hhDFIStaking.connect(addr1).withdraw()).to.be.revertedWith(
                "DFIStaking: not enough tokens",
            );
        });
        ...
        it("Should revert when withdraw ERC20InsufficientAllowance", async () => {
            ...
            await expect(hhDFIStaking.connect(addr1).withdraw()).to.be.revertedWithCustomError(
                erc20Token,
