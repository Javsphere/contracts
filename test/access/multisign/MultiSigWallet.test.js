...
describe("MultiSigWallet contract", () => {
    ...
    before(async () => {
        ...
        required = 2;
        hhMultiSigWallet = await multiSigWallet.deploy(
            owners,
            required
        );
    ...
    describe("Transactions", () => {
        it("Should revert when submitTransaction not owner", async () => {
            ...
            await expect(
                ...
                "MultiSigWallet: owner does not exist"
            );
        ...
        it("Should confirmTransaction", async () => {
            ...
            await expect(await hhMultiSigWallet.getConfirmationCount(trxId)).to.be.equal(prevConfirmations + BigInt(1));
        });
        ...
        it("Should executeTransaction", async () => {
            ...
            await expect(await erc20Token.balanceOf(addr1.address)).to.be.equal(addr1BalanceBefore + amount);

        });

        it("Should revokeConfirmation", async () => {
            const data = erc20Token.interface.encodeFunctionData("mint", [addr1.address, ethers.parseEther("3")])
