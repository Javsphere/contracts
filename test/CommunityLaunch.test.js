...
describe("CommunityLaunch contract", () => {
    ...
    describe("Deployment", () => {
        ...
        it("Should set the right multi sign wallet address", async () => {
            await expect(await hhCommunityLaunch.multiSignWallet()).to.equal(
                multiSignWallet.address,
            );
        });
    ...
    describe("Transactions", () => {
        ...
        it("Should revert when withdraw ADMIN_ERROR", async () => {
            ...
            ).to.be.revertedWith("CommunityLaunch: only multi sign wallet");
        });

        it("Should revert when withdraw Invalid amount", async () => {
            await expect(
                hhCommunityLaunch
                    .connect(multiSignWallet)
                    .withdraw(erc20Token.target, addr1.address, ethers.parseEther("100")),
        ...
        it("Should withdraw", async () => {
            ...
            await hhCommunityLaunch
                .connect(multiSignWallet)
                .withdraw(erc20Token.target, addr1.address, amount);
